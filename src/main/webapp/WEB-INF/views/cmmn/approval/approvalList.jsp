	...
	<div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
		<div class="w-full flex justify-between items-center pl-3">
			<div style="margin-top: 30px; margin-bottom: 10px;">
				...
				<p class="text-slate-500">요청된 결재 문서를 쉽게 찾고 편리하게 관리하세요</p>
			</div>
		</div>
	      ...
	      <div class="w-full mt-2 mb-4 grid grid-cols-1 gap-5 md:grid-cols-4 lg:grid-cols-4 2xl:grid-cols-4 3xl:grid-cols-4">
	         ...
	         <div class="relative flex flex-grow !flex-row flex-col items-center rounded-[10px] rounded-[10px] border-[1px] border-gray-200 bg-white bg-clip-border shadow-md shadow-[#F3F3F3] dark:border-[#ffffff33] dark:!bg-navy-800 dark:text-white dark:shadow-none" style="height: 100px;">
	            ...
	            <div class="h-50 ml-4 flex w-auto flex-col justify-center">
	               <p class="request-text">긴급 결재 요청</p>
	               <h4 class="mt-2 text-xl font-bold text-navy-700" id="wCntElement" countto="2"><a class="aBtn" href="/approval/approvalList?emrgncySttus=Y">${approvalLineCountVO.countEmrgncyY}</a><span class="text-span"> 건</span></h4>
	            </div>
	         </div>
	         ...
	         <div class="relative flex flex-grow !flex-row flex-col items-center rounded-[10px] rounded-[10px] border-[1px] border-gray-200 bg-white bg-clip-border shadow-md shadow-[#F3F3F3] dark:border-[#ffffff33] dark:!bg-navy-800 dark:text-white dark:shadow-none" style="height: 100px;">
	            <div class="ml-[18px] flex h-[90px] w-auto flex-row items-center">
	               <div class="rounded-full bg-lightPrimary p-3 dark:bg-navy-700">
	                  <span class="flex items-center text-brand-500">
	                     <i class="fa-solid fa-file fa-2xl" style=""></i>
	                  </span>
	               </div>
	            </div>
	            <div class="h-50 ml-4 flex w-auto flex-col justify-center">
	               <p class="request-text">일반 결재 요청</p>
	               <h4 class="mt-2 text-xl font-bold text-navy-700" id="myNoticesElement" countto="0"><a class="aBtn" href="/approval/approvalList?emrgncySttus=N">${approvalLineCountVO.countEmrgncyN}</a><span class="text-span"> 건</span></h4>
	            </div>
	         </div>
	         ...
	         <div class="relative flex flex-grow !flex-row flex-col items-center rounded-[10px] rounded-[10px] border-[1px] border-gray-200 bg-white bg-clip-border shadow-md shadow-[#F3F3F3] dark:border-[#ffffff33] dark:!bg-navy-800 dark:text-white dark:shadow-none" style="height: 100px;">
	            ...
	            <div class="h-50 ml-4 flex w-auto flex-col justify-center">
	               <p class="request-text">결재 대기</p>
	               <h4 class="mt-2 text-xl font-bold text-navy-700" id="etcElement" countto="1511"><a class="aBtn" href="/approval/approvalList?approveNow=A14-001">${approvalLineCountVO.countApprove}</a><span class="text-span"> 건</span></h4>
	            </div>
	         </div>
	         ...
	         <div class="relative flex flex-grow !flex-row flex-col items-center rounded-[10px] rounded-[10px] border-[1px] border-gray-200 bg-white bg-clip-border shadow-md shadow-[#F3F3F3] dark:border-[#ffffff33] dark:!bg-navy-800 dark:text-white dark:shadow-none" style="height: 100px;">
	            ...
	            <div class="h-50 ml-4 flex w-auto flex-col justify-center">
	               <p class="request-text">결재 예정</p>
	               <h4 class="mt-2 text-xl font-bold text-navy-700" id="etcElement" countto="1511"><a class="aBtn" href="/approval/approvalList?approveNow=A14-003">${approvalLineCountVO.countWait}</a><span class="text-span"> 건</span></h4>
	            </div>
	         </div>
		...
		<div class="flex justify-end mb-3">
		  <div class="flex items-center space-x-2 mr-3 pt-2">
		  ...
		  <form id="searchForm">
		    <div class="flex justify-end space-x-3 mt-2">
		      ...
		      <div class="flex items-center space-x-2">
		        ...
		        <input type="date" id="startDate" name="startDate" class="bg-white w-full h-10 px-2 bg-transparent placeholder:text-slate-400 text-slate-700 text-sm border border-slate-200 rounded transition duration-200 ease focus:outline-none focus:border-slate-400 hover:border-slate-400" value="">
		        		
		        <span>-</span>
		        <input type="date" id="endDate" name="endDate" class="bg-white w-full h-10 px-2 bg-transparent placeholder:text-slate-400 text-slate-700 text-sm border border-slate-200 rounded transition duration-200 ease focus:outline-none focus:border-slate-400 hover:border-slate-400" value="">		        				
		      </div>
		      <!-- 검색 필드 -->
		      <div class="relative">
						...
						<c:choose>
							<c:when test="${not empty approvalDocVOList}">
								<c:forEach var="items" items="${approvalDocVOList}" varStatus="stat">
									<tr>
										<td>${items.approvalLineVOList[0].rn}</td>
							...
							<c:otherwise>
								<tr>
									<td colspan="6">결재 요청된 문서가 없습니다.</td>
								</tr>
							</c:otherwise>
						</c:choose>
					</tbody>
				...
				<ul class="inline-flex space-x-2">
					...
					<li>
						<c:if test="${articlePage.startPage gt 5}">
							<a href="/approval/approvalList?currentPage=${articlePage.startPage-5}" class="flex items-center justify-center w-10 h-10 text-indigo-600 transition-colors duration-150 rounded-full focus:shadow-outline hover:bg-indigo-100">
					...
					<c:forEach var="pNo" begin="${articlePage.startPage}"
						...
						<c:if test="${articlePage.currentPage == pNo}">
							<li>
								<button id="button-${pNo}" onclick="javascript:location.href='/approval/approvalList?currentPage=${pNo}&searchField=${param.searchField}&keyword=${param.keyword}&startDate=${param.startDate}&endDate=${param.endDate}&emrgncySttus=${param.emrgncySttus}&approveNow=${param.approveNow}';" class="w-10 h-10 transition-colors duration-150 rounded-full focus:shadow-outline" style="background-color: #4E7DF4; color: white;">
									${pNo}
								</button>
							</li>
						</c:if>
						...
						<c:if test="${articlePage.currentPage != pNo}">
							<li>
								<button id="button-${pNo}" onclick="javascript:location.href='/approval/approvalList?currentPage=${pNo}&searchField=${param.searchField}&keyword=${param.keyword}&startDate=${param.startDate}&endDate=${param.endDate}&emrgncySttus=${param.emrgncySttus}&approveNow=${param.approveNow}';" class="w-10 h-10 transition-colors duration-150 rounded-full focus:shadow-outline hover:bg-indigo-100" style="color: #4E7DF4;">${pNo}</button>
							</li>
						</c:if>
					</c:forEach>
					...
					<li>
						<c:if test="${articlePage.endPage lt articlePage.totalPages}">
							<a href="/approval/approvalList?currentPage=${articlePage.startPage+5}&searchField=${param.searchField}&keyword=${param.keyword}&startDate=${param.startDate}&endDate=${param.endDate}&emrgncySttus=${param.emrgncySttus}&approveNow=${param.approveNow}';" class="flex items-center justify-center w-10 h-10 text-indigo-600 transition-colors duration-150 rounded-full focus:shadow-outline hover:bg-indigo-100">
