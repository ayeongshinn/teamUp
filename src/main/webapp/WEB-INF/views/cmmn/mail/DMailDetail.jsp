...
<%-- 						<c:out value="${mailDVOList[0]}"></c:out> --%>
						<c:set var="mailDVO" value="${mailDVOList[0]}" />
						...
						<div>    
					        <p>
					            <fmt:formatDate value="${mailDVO.dsptchDt}" pattern="yyyy.MM.dd(E) HH:mm" /><br>
					        </p>
					    </div>
						<div>
					        <p>받는 사람 :
									...
									<c:forEach var="file" items="${fileVOList}">
										<c:choose>
	                                         <c:when test="${fn:endsWith(file.fileOriginalNm, '.pdf')}">
	                                         	<div class="flex">
		                                         	<a href="/resources${file.fileSaveLocate}"  target="_blank" class="viewPdf">${file.fileOriginalNm} [${file.fileFancysize}]</a>
						...
						<div class="flex justify-end p-1" style="gap: 10px;">
							<button type="button" id="btnRegistPost" ondblclick="location.href='/cmmn/mail/sendList?empNo=${empVO.empNo}'"
