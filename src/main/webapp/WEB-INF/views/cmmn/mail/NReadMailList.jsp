...
$(function() {
	...
	///\$\{param\.keyword\}/g는 ${param.keyword}라는 문자열을 찾는 정규 표현식입니다. 정규 표현식에서 $, {, }, . 같은 특수 문자는 백슬래시(\)로 이스케이프 처리가 필요합니다.
...
<div class="max-w-7xl mx-auto sm:px-8 lg:px-10">
		<div class="w-full flex justify-between items-center mt-1 pl-3">
			<div style="margin-top: 30px; margin-bottom: 10px;">
				...
				<p class="text-slate-500">안읽은 메일함</p>
			</div>
								...
								<c:forEach var="mailDVO" items="${mailDVOList}">
									<c:forEach var="mailRVO" items="${mailDVO.mailRVOList}">
										...
										<td style="text-align: left; width: 50%;"><a
											href="/cmmn/mail/detail?mailNo=${mailDVO.mailNo}&rcptnSn=${mailRVO.rcptnSn}">
												<c:set var="word"
													value="<span style='color:#4E7DF4;'>${param.keyword}</span>" />
												<c:set var="resultTitle"
													value="${fn:replace(mailDVO.mailTtl, param.keyword, word)}" />
												<c:if test="${mailDVO.dimprtncYn == 'Y'}">
													<i class="fa-solid fa-exclamation" style="color: #f94e4e;"></i>
												</c:if> <c:if test="${fn:length(resultTitle) > 30}">
					...
					<nav aria-label="Page navigation"
						...
						<ul class="inline-flex space-x-2">
							...
							<li><c:if test="${articlePage.startPage gt 5}">
									<a
										href="/cmmn/mail/NReadMailList?currentPage=${articlePage.startPage-5}&empNo=${empVO.empNo}"
										style="color: #4E7DF4"
								...
								<c:if test="${articlePage.currentPage == pNo}">
									<li>
										<button id="button-${pNo}"
											onclick="javascript:location.href='/cmmn/mail/NReadMailList?currentPage=${pNo}&empNo=${empVO.empNo}';"
