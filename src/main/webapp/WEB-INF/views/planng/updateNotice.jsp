...
<script type="text/javascript">
	...
	$(document).ready(function () {
		...
		$(".remove-file-existing").on("click",function(){
			...
			let str = "<input type='hidden' name='fileDetailVOList["+imageCnt+"].fileSn' value='"+fileSn+"' />";
			    str += "<input type='hidden' name='fileDetailVOList["+imageCnt+"].fileGroupNo' value='"+fileGroupNo+"' />";
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-14 bg-white border-b border-gray-200">
                	...
                	1. <input type="file"... + spring security
                	 ...
                	 -->
                    <form action="/updateNotice?${_csrf.parameterName}=${_csrf.token}" method="POST" enctype="multipart/form-data">
                    	<input type="hidden" name="empNo" value="${empVO.empNo}"/>
                        <input type="hidden" name="ntcNo" value="${param.ntcNo}" />
                        <!-- 이미지 삭제를 위함  -->
                       <span id="spnImgDelete"></span>
                       <div class="mr-4 mb-3">
                        ...
                        <div class="mb-4 flex items-center">
						    ...
						    <input type="text" class="bg-white border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-4/5 p-2.5 mr-4"
						    		name="ntcTtl" id="ntcTtl" placeholder="제목을 입력해 주세요" required value="${noticeVO.ntcTtl}">
						    
						    <div class="flex items-center">
						        <input type="radio" name="imprtncYn" value="N" id="normal" class="mr-1"
							        ${noticeVO.imprtncYn == 'N' ? 'checked="checked"' : ''}>
							    <label for="normal" class="text-md font-medium text-gray-900 ml-2 mr-3 block">일반</label>
							
							    <input type="radio" name="imprtncYn" value="Y" id="important" class="mr-1"
							        ${noticeVO.imprtncYn == 'Y' ? 'checked="checked"' : ''}>
							    <label for="important" class="text-md font-medium text-gray-900 ml-2 block">중요</label>
						    </div>
						</div>
					
					    <div class="mb-8"> 
					        <textarea style="resize: none; overflow-y: auto; height: 400px;" id="ntcCn" name="ntcCn" class="border-2 border-gray-500 w-full"
					        		  placeholder="내용을 입력해 주세요">${noticeVO.ntcCn}</textarea>
						...
						<div class="mb-4 flex items-center">
						    ...
						    <!-- 파일 리스트 영역 -->
						    <div id="fileList" class="text-sm text-gray-500 mb-2">
						        <!-- 기존 파일 목록 출력 -->
